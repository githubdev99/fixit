...
class MY_Controller extends MX_Controller
{
	...
	public function convert_foto($property, $foto, $url)
	{
		if (isset($_POST[$property])) {
			$type = (strpos($foto, 'data:image/jpeg') !== false || strpos($foto, 'data:image/jpg') !== false) ? 'jpeg' : 'png';
			$img = $_POST[$property];
			$img = str_replace('data:image/' . $type . ';base64,', '', $img);
			$img = str_replace(' ', '+', $img);
			$data = base64_decode($img);
			$unik = uniqid();
			$nama_file = date("Ymd") . $unik . '.' . $type;
			$file = './' . $url . date("Ymd") . $unik . '.' . $type;
	...
	public function ReadNotif($param, $table, $field_data, $field_where)
	{
		if (!empty($param)) {
			$cek = $this->master_model->GetJumlahDataFilter($table, array($field_where => $param));
			if ($cek == 1) {

				// Read to message
				// $sql = $this->proses->UpdateData($table, array($field_data => 'Y'), array($field_where => $param));

				$sql = $this->master_model->send_data([
					'where' => [
						$field_where => $param
					],
					'data' => [
