<script>
    $(document).ready(function() {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                id_kategori: '<?= encrypt_text($get_data['master']->id_kategori) ?>'
            },
            dataType: 'json',
        ...
        <?php if (strtolower($get_data['master']->variasi_harga) == 'y') : ?>
            ...
            $('input[id="counter_variasi"]').val(<?= $get_data['variasi_total'] ?>);
        ...
        <?php if (!empty($get_data['master']->id_sub_kategori)) : ?>
            $.ajax({
                url: '<?= base_url() ?>json/option_sub_kategori_produk',
                type: 'POST',
                data: {
                    id_parent: '<?= encrypt_text($get_data['master']->id_kategori) ?>',
                    id_sub_kategori: '<?= encrypt_text($get_data['master']->id_sub_kategori) ?>'
                },
                dataType: 'json',
                success: function(response) {
                    var data = response.data;
        ...
        <?php if (strtolower($get_data['master']->pre_order) == 'y') : ?>
        ...
        <?php $no_foto = 1;
        foreach ($get_data['foto'] as $key) : ?>
            <?php if (!empty($key->foto)) : ?>
                $('input[name="id_foto_<?= $no_foto ?>"]').val('<?= encrypt_text($key->id_foto) ?>');
                $('input[name="file_foto_<?= $no_foto ?>"]').val('<?= $key->foto ?>');
                $('#plus_<?= $no_foto ?>').hide();
                $('#FileProduct_<?= $no_foto ?>').attr('disabled', 'true');
                $('#output_<?= $no_foto ?>').css('display', 'block');
                $('#output_<?= $no_foto ?>').attr('src', '<?= $core['folder_upload'] ?>products/<?= $key->foto ?>');
                $('#BottomProduk_<?= $no_foto ?>').show();
                document.getElementById('CekFoto_<?= $no_foto ?>').value = 'Y';
            <?php endif ?>
        <?php $no_foto++;
        endforeach ?>

        <?php $no_variasi = 1;
        foreach ($get_data['variasi'] as $value_variasi) : ?>
            <?php if (!empty($value_variasi->model)) : ?>
                var value_model = 'model';
                copying_variasi(<?= $no_variasi ?>, '<?= $value_variasi->model ?>');
            <?php elseif (!empty($value_variasi->warna)) : ?>
                var value_model = 'warna';
                show_model_edit(<?= $no_variasi ?>, value_model, '<?= $value_variasi->warna ?>');
                copying_variasi(<?= $no_variasi ?>, '<?= $value_variasi->warna ?>');
            <?php elseif (!empty($value_variasi->ukuran)) : ?>
                var value_model = 'ukuran';
                show_model_edit(<?= $no_variasi ?>, value_model, '<?= $value_variasi->ukuran ?>');
                copying_variasi(<?= $no_variasi ?>, '<?= $value_variasi->ukuran ?>');
            <?php endif ?>
        <?php $no_variasi++;
        endforeach ?>
    ...
    function show_model_edit(param, value, selected) {
        $('#daftar_model_' + param + ' select').html('<option></option>');
        $('#read_nama_' + param).html('');
        if (value != 'model') {
            $('#daftar_model_' + param).show();
    ...
    function load_all_option() {
        ...
        <?php if (!empty($get_data['master']->merek)) : ?>
            $('select[name="merek"]').html(new Option('<?= $get_data['master']->nama_merek ?>', '<?= encrypt_text($get_data['master']->merek) ?>', false, false)).trigger('change');
        <?php endif ?>
        ...
        <?php if (!empty($get_data['master']->asal_produk)) : ?>
            $('select[name="asal_produk"]').html(new Option('<?= $get_data['master']->nama_asal_produk ?>', '<?= encrypt_text($get_data['master']->asal_produk) ?>', false, false)).trigger('change');
        <?php endif ?>
    }
</script>