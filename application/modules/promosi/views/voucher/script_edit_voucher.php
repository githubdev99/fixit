<script>
    $(document).ready(function () {
        <?php if (!empty($get_data->id_sub_kategori)): ?>
			$.ajax({
                url: '<?= base_url() ?>json/option_sub_kategori_produk',
                type: 'POST',
                data: {
                    id_parent: '<?= encrypt_text($get_data->id_kategori) ?>',
        ...
        <?php if (!empty($get_data->banner_promo)): ?>
            $('#plus_1').hide();
            $('#FileProduct_1').attr('disabled', 'true');
			$('#output_1').css('display','block');
            $('#output_1').attr('src','<?= $core['folder_upload_other'] ?>file/<?= $get_data->banner_promo ?>');
			$('#BottomProduk_1').show();
			document.getElementById('CekFoto_1').value = 'Y';
        <?php endif ?>

        if ($('input[name="tipe_promo"]:checked').val() == 'yes') {
        ...
        if ($('select[name="tipe_diskon"] option:selected').val() == 'nominal') {
            $('#dalam_rupiah').show();
